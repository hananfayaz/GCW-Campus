...
import com.example.gcwcampus.models.FirebaseFacultyModel;
...
public class FacultyCreateProfileActivity extends AppCompatActivity {
    ...
    String name, address, dob, department, email, password, confirmPassword, genderMale, genderFemale, gender;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sprDept = findViewById(R.id.edt_txt_dept);
        ...
        sprDept.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
        ...
        btnSubmit.setOnClickListener(v -> {
            ...
            if (!password.equals(confirmPassword)){
                ...
                FacultyEntity facultyEntity = new FacultyEntity(name, address, phNumber, dob, department, email, gender, password);
                executor.submit(() -> {
                    try {
                        gcwDatabaseHelper.facultyDao().addFacultyInfo(facultyEntity);
                    } catch (Exception e) {
                ...
                executor.submit(() -> {
                    try {
                        ...
                        databaseReference.child(department).child(phNumber).setValue(new FirebaseFacultyModel(name, address, dob, email, gender, password));
                    } catch (Exception e) {
                        Toast.makeText(this, "Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                startActivity(new Intent(FacultyCreateProfileActivity.this, DashboardActivity.class));
                finish();
            }
        });
    ...
    private void updateFormValidity(){
        isFormValid = !edtTxtName.getText().toString().isEmpty() &&
                ...
                sprDept.getSelectedItemPosition() > 0 &&
                isGenderChecked;
